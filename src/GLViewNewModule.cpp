...
using namespace std;
 ...
 vec3df CamDirection(Aftr::Vector cameraDirection) {
	...
	float positionY = cameraDirection[1];
	float positionZ = cameraDirection[2];
	irrklang::vec3df newdir(positionX, positionY, positionZ);
	return newdir;
}
//
vec3df CamPosition(Vector cameraPosition) {
	float positionX = cameraPosition[0];
	float positionY = cameraPosition[1];
	float positionZ = cameraPosition[2];
	irrklang::vec3df newpos(positionX, positionY, positionZ);
	return newpos;
}

vec3df CamNorm(Vector cameraNormal) {
	float positionX = cameraNormal[0];
	float positionY = cameraNormal[1];
	float positionZ = cameraNormal[2];
	irrklang::vec3df newnorm(positionX, positionY, positionZ);
	return newnorm;
}
...
void GLViewNewModule::createNewModuleWayPoints()
{
   ...
   wayPt->setPosition( Vector( 50, 0, 100 ) );
